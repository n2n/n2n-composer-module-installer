...
class ModuleInstallerComposer2 extends LibraryInstaller {
	/**
	 * {@inheritDoc}
	...
	public function install(\Composer\Repository\InstalledRepositoryInterface $repo, \Composer\Package\PackageInterface $package) {
		...
		return parent::install($repo, $package)->then(function() use ($package, $that) {
			$that->removeResources($package);
			$that->installResources($package);
		});
	...
	public function update(\Composer\Repository\InstalledRepositoryInterface $repo, \Composer\Package\PackageInterface $initial, 
    	...
    	$that = $this;
		return parent::update($repo, $initial, $target)->then(function() use ($that, $target) {
			$that->removeResources($target);
			$that->installResources($target);
		});
	}
